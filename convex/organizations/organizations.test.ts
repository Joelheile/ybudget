...
test("get organization name", async () => {
  ...
  const name = await user.query(api.organizations.queries.getOrganizationName, {});
...
test("get user organization", async () => {
  ...
  const user = test.withIdentity({ subject: userId });
  const org = await user.query(api.organizations.queries.getUserOrganization, {});
...
test("create organization", async () => {
  ...
  const organizationId = await test.mutation(api.organizations.functions.createOrganization, {
    name: "New Org",
    domain: "test.com",
    userId,
  });
  ...
  const projects = await test.run(async (ctx) =>
    ctx.db.query("projects").withIndex("by_organization", (q) => q.eq("organizationId", organizationId)).collect()
  );
