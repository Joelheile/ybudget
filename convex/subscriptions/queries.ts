...
export const getSubscriptionStatus = query({
  ...
  handler: async (ctx) => {
    ...
    const now = Date.now();
    const trialEndsAt = organization.trialEndsAt || organization._creationTime + (14 * 24 * 60 * 60 * 1000);
    const daysLeftInTrial = Math.max(0, Math.ceil((trialEndsAt - now) / (24 * 60 * 60 * 1000)));
    ...
    return {
      status: hasAccess ? subscriptionStatus : (subscriptionStatus === "trial" ? "expired" : subscriptionStatus),
      hasAccess,
      tier: organization.subscriptionTier,
      trialEndsAt,
      daysLeftInTrial: subscriptionStatus === "trial" ? daysLeftInTrial : undefined,
    };
  },
});
