...
export async function generateReimbursementPDF(
  ...
  coverPage.drawText(`Belege: ${receipts.length}`, {
    x: 50,
    y,
    size: 10,
    font,
  ...
  for (const receipt of receipts) {
    ...
    infoY -= 40;
    infoPage.drawText(`Datum: ${receipt.receiptDate || ""}`, {
      x: 50,
      y: infoY,
      size: 10,
      font,
    ...
    infoY -= 20;
    infoPage.drawText(`Firma: ${receipt.companyName || ""}`, {
      x: 50,
      y: infoY,
      size: 10,
      font,
    ...
    infoY -= 20;
    infoPage.drawText(`${receipt.description || ""}`, {
      x: 50,
      y: infoY,
      size: 10,
      font,
    ...
    infoY -= 20;
    infoPage.drawText(`Betrag: ${(receipt.grossAmount || 0).toFixed(2)} €`, {
      x: 50,
      y: infoY,
      size: 10,
      font: boldFont,
