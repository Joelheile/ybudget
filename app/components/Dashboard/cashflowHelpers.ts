...
export function determinePeriods(rangeStart: Date, rangeEnd: Date): Period[] {
  ...
  if (monthsDiff >= 11 || daysDiff >= 335) {
    return eachMonthOfInterval({ start: rangeStart, end: rangeEnd }).map(
      (monthDate) => {
...
export function aggregateTransactionsForPeriod(
  ...
  return {
    processedIncome: processed.reduce(
      (sum, t) => sum + (t.amount > 0 ? t.amount : 0),
      0,
    ),
    expectedIncome: expected.reduce(
      (sum, t) => sum + (t.amount > 0 ? t.amount : 0),
      0,
    ),
    processedExpenses: processed.reduce(
      (sum, t) => sum + (t.amount < 0 ? Math.abs(t.amount) : 0),
      0,
    ),
    expectedExpenses: expected.reduce(
      (sum, t) => sum + (t.amount < 0 ? Math.abs(t.amount) : 0),
      0,
    ),
  };
...
export function calculatePeriodBalance(
  ...
  return allTransactions.reduce((balance, transaction) => {
    return transaction.date < periodEndTime
      ? balance + transaction.amount
      : balance;
  }, startBalance);
}

export function findMaxBarValue(
  dataPoints: Array<{
    actualIncome: number;
    expectedIncome: number;
...
export function findBalanceRange(dataPoints: Array<{ balance: number }>): {
  min: number;
  max: number;
} {
