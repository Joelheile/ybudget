...
function getEditState(row: any, table: any) {
  ...
  const isPlanned = row.original.status === "expected";
  const isEditing = table.options.meta?.editingRows?.has(rowId);
  const onUpdate = (field: string, value: any) => table.options.meta?.onUpdate(rowId, field, value);
...
const baseColumns = [
  ...
  },
  {
    ...
    cell: ({ row, table }: any) => {
      ...
      if (isPlanned && isEditing) {
        return <div className="pl-2"><EditableDateCell value={row.getValue("date")} onSave={(v) => onUpdate("date", v)} /></div>;
      }
  ...
  },
  {
    ...
    cell: ({ row, table }: any) => {
      const { isPlanned, isEditing, onUpdate } = getEditState(row, table);
      if (isPlanned && isEditing) {
        return <EditableProjectCell value={row.original.projectId} onSave={(v) => onUpdate("projectId", v)} />;
      }
  ...
  },
  {
    ...
    cell: ({ row, table }: any) => {
      ...
      if (isEditing) {
        return <EditableTextareaCell value={description} onSave={(v) => onUpdate("description", v)} />;
      }
      return (
        <div className="min-w-64 max-w-xl">
          <div className="whitespace-pre-wrap text-muted-foreground wrap-break-word text-sm">{description}</div>
        </div>
      );
    },
  },
  {
    ...
    cell: ({ row, table }: any) => {
      const { isEditing, onUpdate } = getEditState(row, table);
      if (isEditing) {
        return <EditableCategoryCell value={row.original.categoryId} onSave={(v) => onUpdate("categoryId", v)} />;
      }
  ...
  {
    accessorKey: "amount",
    header: ({ column }: any) => <div className="flex justify-end"><SortableHeader column={column} label="Betrag" /></div>,
    cell: ({ row, table }: any) => {
      const { isPlanned, isEditing, onUpdate } = getEditState(row, table);
      const amount = row.getValue("amount");
      if (isPlanned && isEditing) {
        return <div className="flex justify-end pr-2"><EditableAmountCell value={amount} onSave={(v) => onUpdate("amount", v)} /></div>;
      }
  ...
  },
  {
    ...
    cell: ({ row }: any) => {
      ...
      const label = isBudgetSplit ? "Budget" : status === "expected" ? "Geplant" : "Abgerechnet";
      const variant = isBudgetSplit ? "outline" : status === "processed" ? "default" : "secondary";
